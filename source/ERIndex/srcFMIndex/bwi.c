...
/* ***************************************************************
        Main bwi procedure
   *************************************************************** */
//used to build an FM-index v1 (removed decompression features, not required in this context)
int bwiMain(int argc, char *argv[])
{
  ...
  start = getTime();
  bwi_open_files(infile_name,outfile_name,decompress);
  //if(decompress)
  //  decompress_file();
  //else
  compress_file(0);
...
/* ***************************************************************
   ...
   *************************************************************** */
void bwi_open_files(char *infile_name, char *outfile_name, int decompress)
{
  ...
  if(infile_name==NULL)
    ...
    Infile=orig_my_fopen( infile_name, "rb"); // b is for binary: required by DOS
    if(Infile==NULL) {
  ...
  if(!decompress) {
    if(outfile_name!=NULL) {
      strip_ext(outfile_name);
      Safile_name = (char *) malloc(strlen(outfile_name)+4);
      Safile_name = strcpy(Safile_name, outfile_name);
		...
		int save_sa_to_disk)
{
  ...
  Bucket_size_lev1=bs_lev1;
  Bucket_size_lev2=bs_lev2;
  Marked_char_freq=mc_freq;
  ...
  start = getTime();
  bwi_open_files(infile_name,outfile_name,0);  //last argument: decompress=0
  //if(decompress)
  //  decompress_file();
  //else
